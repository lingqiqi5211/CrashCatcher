...
/// The generated `BridgeVersion`, carrying the same label as `module.prop`.
fn bridge_version_source(workspace: &Path) -> Result<String, PackagerError> {
    let version = version_label(workspace)?;
    Ok(format!(
        "package io.github.lingqiqi5211.crashcatcher.bridge;\n\
         \n\
         /** Generated by cch-packager. */\n\
         final class BridgeVersion {{\n\
         \x20   static final String VALUE = \"{version}\";\n\
         \n\
         \x20   private BridgeVersion() {{}}\n\
         }}\n"
    ))
}
...
fn compile_bridge(
    ...
    // The bridge reports its version in the hello frame, and the daemon shows it on the
    // diagnostics page. Java has no preprocessor, so the checked-in constant is a placeholder
    // and the real one is generated here — otherwise the bridge kept claiming the version it was
    // first written at while the daemon beside it moved on, which reads as a stale dex.
    let generated = temporary.path().join("generated");
    fs::create_dir_all(&generated)?;
    let version_source = generated.join("BridgeVersion.java");
    fs::write(&version_source, bridge_version_source(workspace)?)?;
    let sources: Vec<PathBuf> = sources
        .into_iter()
        .filter(|path| path.file_name() != Some(OsStr::new("BridgeVersion.java")))
        .chain([version_source])
        .collect();
